...
class QuestionBase(BaseModel):
    ...
    questionText: Optional[str] = Field(
        None, description="The main prompt or question for the student (can be LocalizedString in some cases)."
    )
    explanation: Optional[LocalizedString] = Field(
        None, description="Explanation for the correct answer."
    )
    createdAt: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Timestamp of creation in UTC."
    )
    updatedAt: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Timestamp of last update in UTC."
    )
...
class AssetBase(BaseModel):
    ...
    assetId: str = Field(description="Unique ID for the asset, typically a UUID hex string.")
    title: LocalizedString
    description: Optional[LocalizedString] = Field(None, description="A short description of the asset.")
    difficulty: DifficultyDetail # Difficulty level associated with the asset itself
    learningObjectives: List[str] = Field(default_factory=list, description="Learning objectives covered by this asset.")
    tags: List[str] = Field(default_factory=list, description="Keywords or tags associated with the asset.")
    status: Literal["DRAFT", "PUBLISHED", "ARCHIVED"] = Field("DRAFT", description="Publication status of the asset.")
    ...
    createdBy: Optional[str] = Field(None, description="Identifier of the user/process that created this asset.")
    createdAt: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Timestamp of creation in UTC."
    )
    updatedAt: datetime = Field(
...
class FillInTheBlankQuestion(QuestionBase):
    ...
    questionText: str  # Overriding to make it mandatory and not LocalizedString for this type
...
class TranslationQuestion(QuestionBase):
    ...
    targetLanguage: Literal["en", "zh_tw"] = Field(description="The language to translate into.")
...
class PictureDescriptionQuestion(QuestionBase):
    ...
    imageAssetId: str = Field(description="ID of the AssetDocument representing the image.")
    ...
    suggestedKeywords: Optional[List[str]] = Field(None, description="Keywords that might be expected in the description.")
...
class ReadingComprehensionQuestion(QuestionBase):
    ...
    def check_answers_logic(self) -> "ReadingComprehensionQuestion":
        ...
        if self.choices is not None:
            correct_choices_count = sum(1 for choice in self.choices if choice.isCorrect)
            if correct_choices_count != 1:
                raise ValueError("For multiple-choice READING_COMPREHENSION questions, exactly one choice must be correct.")
        return self
...
class ListeningComprehensionQuestion(QuestionBase):
    ...
    audioAssetId: str = Field(description="ID of the AssetDocument (e.g., an audio clip).")
    ...
    def check_answers_logic(self) -> "ListeningComprehensionQuestion":
        ...
        if self.choices is not None:
            correct_choices_count = sum(1 for choice in self.choices if choice.isCorrect)
            if correct_choices_count != 1:
                raise ValueError("For multiple-choice LISTENING_COMPREHENSION questions, exactly one choice must be correct.")
        return self
...
class SpellingCorrectionQuestion(QuestionBase):
    ...
    wordChoices: Optional[List[str]] = Field(
        None, description="List of words, one of which is correctly spelled (or one misspelled)."
    )
    correctWord: Optional[str] = Field(
        None, description="The correctly spelled word among the choices, or the correct spelling if sentence-based."
    )
    ...
    misspelledWordInSentence: Optional[str] = Field(
        None, description="The specific word that is misspelled in the sentence (if applicable)."
    )
    ...
    def check_spelling_correction_logic(self) -> "SpellingCorrectionQuestion":
        ...
        if self.wordChoices:
            if not self.correctWord:
                raise ValueError("If 'wordChoices' is provided, 'correctWord' must also be provided.")
            ...
            raise ValueError("For SPELLING_CORRECTION, either 'wordChoices' or 'sentenceWithMisspelledWord' setup must be provided.")
        return self
...
class AudioAsset(AssetBase):
    ...
    durationSeconds: Optional[float] = Field(None, description="Duration of the audio in seconds.")
    transcript: Optional[str] = Field(None, description="Full transcript of the audio content, if available.")
    speakerInfo: Optional[List[str]] = Field(None, description="Information about speakers, if multiple.")
