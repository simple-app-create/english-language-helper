...
from sections import welcome_tab, reading_tab, saved_words_tab, questions_tab, listening_tab
    ...
    try:
        ...
        if not firebase_admin._apps:
            ...
            if service_account_json_str:
                try:
                    ...
                    if cred_object:
                         ...
                         main_app_logger.info("Attempting to initialize Firebase with FIREBASE_SERVICE_ACCOUNT_JSON env var.")
                    else: # If json.loads returns None or empty dict
                        main_app_logger.warning("FIREBASE_SERVICE_ACCOUNT_JSON was found but was empty or invalid after parsing.")
                except json.JSONDecodeError as e:
                    main_app_logger.error(f"Failed to parse FIREBASE_SERVICE_ACCOUNT_JSON: {e}", exc_info=True)
                    service_account_json_str = None # Ensure it falls through or is handled
                except Exception as e: # Catch other potential errors with Certificate creation
                    main_app_logger.error(f"Error creating credential from FIREBASE_SERVICE_ACCOUNT_JSON: {e}", exc_info=True)
            ...
            if not cred_object: # If not loaded from direct JSON string
                service_account_file_path = os.getenv("FIREBASE_SERVICE_ACCOUNT_KEY_PATH")
                if service_account_file_path:
                    if os.path.exists(service_account_file_path):
                        ...
                        main_app_logger.info(f"Attempting to initialize Firebase with FIREBASE_SERVICE_ACCOUNT_KEY_PATH: {service_account_file_path}")
                    else:
                        main_app_logger.error(f"FIREBASE_SERVICE_ACCOUNT_KEY_PATH path not found: {service_account_file_path}")
                        st.error("Firebase configuration error (file path). See app.log for details.")
                        return None
                else:
                    main_app_logger.error("Firebase credentials (FIREBASE_SERVICE_ACCOUNT_JSON or FIREBASE_SERVICE_ACCOUNT_KEY_PATH) not found in environment variables.")
                    st.error("Firebase configuration error (credentials not set). See app.log for details.")
                    return None
        ...
        main_app_logger.critical(f"Critical error during Firebase initialization: {e}", exc_info=True)
...
def main() -> None:
    ...
    tab_labels: list[str] = ["歡迎", "閱讀文章", "單字練習", "聽力練習", "測驗"] # "歡迎" is Welcome

    # Create the tabs
    tab_welcome, tab_reading, tab_saved_words, tab_listening, tab_questions = st.tabs(tab_labels)

    # Call functions from separate files to display tab content
    welcome_tab.show_welcome_tab(tab_welcome)
    # Pass the db_client to the reading_tab function
    reading_tab.show_reading_tab(tab_reading, db_client)
    saved_words_tab.show_saved_words_tab(tab_saved_words)
    listening_tab.show_listening_tab(tab_listening)
    # Pass the db_client to the questions_tab function
    questions_tab.show_questions_tab(tab_questions, db_client)
